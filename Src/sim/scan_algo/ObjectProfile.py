...
class RotationallySymmetric(ObjectProfile):
    ...
    def update(self, time_elapsed):
        ...
        cur_flow = self.flow[self.cur_flow_idx] # set the current action using the current flow index (which is incremented after an action is complete)
        ...
        if isinstance(cur_flow, Wait): # if the current action is to wait
            ...
            if cur_flow.start_time == 0:
                cur_flow.start_time = time_elapsed
                cur_flow.set_stepper(self.sim.robot_handler.stepper_board) # set the stepper board to the one instatiated at start of program
            ...
            if cur_flow.start_time == 0: # at the start of the action
                ...
                if not self.cur_slice:
                    ...
                    closest = find_closest_point(self.cur_slice[1], pp.get_link_pose(self.sim.sim_robot, 6)[0]) # in the slice (which contains a bunch of points) find the nearest point from the probe to any point to determine a start location.

                    self.cur_path = find_alignment_point_path(closest, self.cur_slice[1]) # using closest point, find least cost path and set that to our current path
                    self.sim.parent.plot_slice(self.cur_slice[1], self.cur_path) # plot this slice on the GUI for the user
